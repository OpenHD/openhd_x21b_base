...
#include "rkmuxer.h"
#else
#include "../../common_algorithm/misc/include/rkmuxer.h"
#endif

extern int optind;
extern char *optarg;

static RKADK_CHAR optstr[] = "a:I:p:m:c:t:kTPeh";
...
static void print_usage(const RKADK_CHAR *name) {
  ...
  printf("\t-T: enable third-party muxer, Default: disable\n");
#ifdef ENABLE_EIS
...
static int muxerInitTest(int muxerId, char *format, const char *filePath,
                            VideoParam *videoParam, AudioParam *audioParam) {
  RKADK_LOGP("muxerId[%d] test third muxer init", muxerId);
  return rkmuxer_init(muxerId, format, filePath, videoParam, audioParam);
}

static int muxerDeinitTest(int muxerId) {
  RKADK_LOGP("muxerId[%d] test third muxer deinit", muxerId);
  return rkmuxer_deinit(muxerId);
}

static int muxerWriteVideoTest(int muxerId, unsigned char *buffer, unsigned int bufferSize,
                              int64_t pts, int keyFrame) {
  return rkmuxer_write_video_frame(muxerId, buffer, bufferSize, pts, keyFrame);
}

static int muxerWriteAudioTest(int muxerId, unsigned char *buffer, unsigned int bufferSize, int64_t pts) {
  return rkmuxer_write_audio_frame(muxerId, buffer, bufferSize, pts);
}

long long int muxerGetThumbPosTest(int muxerId) {
  RKADK_LOGP("muxerId[%d] test third muxer get thumb pos", muxerId);
  return rkmuxer_get_thumb_pos(muxerId);
}
...
int main(int argc, char *argv[]) {
  ...
  // third-party muxer
  bool bEnableThirdMuxer = false;
  RKADK_THIRD_MUXER_ATTR_S stThirdMuxerAttr;
...
record:
  ...
  if (bEnableThirdMuxer) {
    memset(&stThirdMuxerAttr, 0, sizeof(RKADK_THIRD_MUXER_ATTR_S));
    stThirdMuxerAttr.pfnMuxerInit = muxerInitTest;
    stThirdMuxerAttr.pfnMuxerDeinit = muxerDeinitTest;
    stThirdMuxerAttr.pfnWriteVideo = muxerWriteVideoTest;
    stThirdMuxerAttr.pfnWriteAudio = muxerWriteAudioTest;
    stThirdMuxerAttr.pfnGetThumbPos = muxerGetThumbPosTest;
    stRecAttr.pstThirdMuxerAttr = &stThirdMuxerAttr;
  }
  ...
  while (!is_quit) {
    ...
    if (strstr(cmd, "quit") || is_quit) {
      ...
      goto record;
    } else if (strstr(cmd, "unmute")) {
      bMute = false;
      RKADK_PARAM_SetCamParam(s32CamId, RKADK_PARAM_TYPE_MUTE, &bMute);
    } else if (strstr(cmd, "mute")) {
      bMute = true;
      RKADK_PARAM_SetCamParam(s32CamId, RKADK_PARAM_TYPE_MUTE, &bMute);
    }
