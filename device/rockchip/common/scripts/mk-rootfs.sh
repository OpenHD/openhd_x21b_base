...
usage_hook()
{
	...
	usage_oneline "jammy" "build ubutnu jammy rootfs"
}
...
BUILD_CMDS="rootfs buildroot debian yocto jammy"
build_hook()
{
	check_config RK_ROOTFS || false
	...
	#
	if [ "$ROOTFS" = jammy ]; then
		build_ubuntu_hook $@
		ln -rsf  ./ubuntu/ubuntu-base-jammy.img ./rockdev/rootfs.img
...
#BUILD_UBUNTU_CMDS="ubuntu focal jammy"
build_ubuntu_hook(){
