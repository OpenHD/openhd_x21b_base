...
| V1.7.0     | Fenrir Lin | 2025-01-06   | 新增RK3576 DV的流程框图                                      |
| V1.7.1     | Yu Zheng   | 2025-02-12   | 修改RK3576DV的流程框图和部分参数说明<br/>增加播放显示模块API介绍 |
| V1.7.2     | Fenrir Lin | 2025-03-25   | 新增RV1126B IPC的流程框图                                    |
| V1.7.3     | Fenrir Lin | 2025-04-03   | 新增RV1126B Dual-IPC的流程框图                               |
| V1.7.4     | Yu Zheng   | 2025-04-16   | 新增RV1126B DV的流程框图                                     |
| V1.7.5     | Fenrir Lin | 2025-04-24   | 新增UVC模块相关说明                                          |
| V1.7.6     | Fenrir Lin | 2025-07-02   | 新增RV1126B IPC的media node映射表                            |
...
| rv1126_ipc_rkmedia        | easymedia、rkaiq | 针对RV1126/RV1109平台的IPC产品，使用rkmedia，支持网页和rtsp/rtmp预览，参数动态修改。 |
| rv1126_ipc_rockit         | rockit、rkaiq    | 针对RV1126/RV1109平台的IPC产品，使用rockit，支持网页和rtsp/rtmp预览，参数动态修改。 |
...
| rk3576_dv                 | rockit、rkaiq    | 针对RK3576平台的运动DV产品，支持本地拍照/录像，解码播放，屏幕显示。 |
| rv1126b_ipc               | rockit、rkaiq    | 针对RV1126B平台的单目IPC产品，支持网页和rtsp/rtmp预览，参数动态修改。 |
| rv1126b_dual_ipc          | rockit、rkaiq    | 针对RV1126B平台的双目IPC产品，支持网页和rtsp/rtmp预览，参数动态修改。 |
| rv1126b_dv                | rockit、rkaiq    | 针对RV1126B平台的运动DV产品，支持本地拍照/录像，解码播放，屏幕显示。 |
...
**默认FEC close，当isp.0.enhancement:distortion_correction=FEC，会走不一样的数据流。**
...
RV1126B中，rockit和media节点的对应关系如下表，**vi和vpss不能同时使用同一个media节点**，具体各个通道性能限制请查阅rockit文档。

| rockit ext chn | rockit vi | rockit vpss | media node     |
| -------------- | --------- | ----------- | -------------- |
|                | 0         |             | rkisp_mainpath |
|                | 1         |             | rkisp_selfpath |
| extChn[0]      | 2         | 0           | rkvpss_scale1  |
| extChn[1]      | 3         | 1           | rkvpss_scale0  |
| extChn[2]      | 4         | 2           | rkvpss_scale2  |
| extChn[3]      | 5         | 3           | rkvpss_scale3  |
| extChn[4]      | 6         | 4           | rkvpss_scale4  |
| extChn[5]      | 7         | 5           | rkvpss_scale5  |

注意：需使用RK_MPI_VI_SetModParam指定rockit的六个扩展通道(即rkvpss_scale的六个节点)分配给vi还是vpss，例如以下代码将rkvpss_scale1分配给vpss模块通道0使用，而vi模块此时不能使用通道2。
...
stModParam.stExtChnParam.extChn[0] = 1;
// extChn[1]~[5] = 0;
...
graph LR
       ...
       VI_1_0--TILE-->AVS
       AVS--RFBC-->VPSS_GRP
       VPSS_GRP-->VPSS_CHN_0--RFBC-->VENC_0_draw_nn_result-->RTSP_0
       VENC_0_draw_nn_result--combo-->JPEG
       VPSS_GRP-->VPSS_CHN_1-->VENC_1-->RTSP_1
       VPSS_GRP-->VPSS_CHN_2-->NPU
	   VPSS_CHN_2-->IVS_for_OD_MD
	   VPSS_GRP-->VPSS_CHN_3-->VO
```
...
| 函数名称                     | 功能                     |
| ---------------------------- | ------------------------ |
| rk_player_create             | 创建播放模块             |
| rk_player_destroy            | 销毁播放模块             |
| rk_player_play               | 开始播放                 |
| rk_player_pause              | 暂停播放                 |
| rk_player_stop               | 结束播放                 |
| rk_player_set_file           | 设置播放文件             |
| rk_player_get_photo          | 解析一张jpeg图片         |
| rk_player_release_photo      | 释放解析图片所使用的资源 |
| rk_player_video_seek         | 设置视频播放位置         |
| rk_player_get_video_duration | 获取视频总时长           |
| rk_player_get_video_position | 获取视频当前播放位置     |
| rk_player_audio_seek         | 设置音频播放位置         |
| rk_player_get_audio_duration | 获取音频总时长           |
| rk_player_get_audio_position | 获取音频当前播放位置     |

### UVC模块

重要脚本位于：common\uvc\rkipc_usb_config.sh，默认只跑一路UVC，只支持一种分辨率和格式。如果需要多路和多种格式，请参考脚本中以下判断，自行添加。
...
使用NV12格式，需要修改/data/rkipc.ini中以下配置：
...
enable_compress = 0
[video.0]
output_data_type = NV12
```

使用bulk传输，需要修改common/uvc/rkipc_usb_config.sh

```shell
# echo 2 > ${USB_FUNCTIONS_DIR}/$UVC_GS/uvc_num_request
  echo 5 > ${USB_FUNCTIONS_DIR}/$UVC_GS/streaming_maxburst
  echo 1 > /sys/kernel/config/usb_gadget/rockchip/functions/$UVC_GS/streaming_bulk
```

common/uvc/uvc-gadget.c中使能bulk_mode
...
min_rate = 200 ; 最小码率，详细请参考rockit文档说明
output_data_type = H.265 ; 视频编码类型
smart = open ; 是否开启智能编码，注意不是smartp
...
boundary = 0 ; 对齐边界
font_path = /oem/usr/share/SourceHanSansEN.ttf ; 字库路径
...
height = 320 ; 显示设备高
fps = 30 ; 显示帧率
intf_type = LCD ; 显示设备类型
play_chn_id = 1 ; 播放通道id
play_chn_priority = 2 ; 播放通道的优先级，高优先级的通道会覆盖低优先级通道
```
