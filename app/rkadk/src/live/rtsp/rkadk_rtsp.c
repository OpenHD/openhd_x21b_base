...
typedef struct {
  ...
  bool bWaitIDR;
  bool bVencChnMux;
  RKADK_U64 u64FirstVideoPts;
  RKADK_S64 s64FirstDiffPts;
  RKADK_U32 u32CamId;
...
static int RKADK_RTSP_AudioSetChn(MPP_CHN_S *pstAiChn, MPP_CHN_S *pstAencChn) {
  RKADK_U32 u32AencChnId = 0;
  ...
  pstSensorCfg = RKADK_PARAM_GetSensorCfg(pstCommCfg->sensor_count - 1);
  if (!pstSensorCfg) {
    RKADK_LOGE("RKADK_PARAM_GetSensorCfg[%d] failed", pstCommCfg->sensor_count - 1);
    return -1;
  }

  u32AencChnId = pstSensorCfg->aenc_chn + 1;
  if (u32AencChnId >= AENC_MAX_CHN_NUM) {
    RKADK_LOGE("invalid u32AencChnId[%d], range[0, %d)", u32AencChnId, AENC_MAX_CHN_NUM);
    return -1;
  }

  for (int i = 0; i < pstCommCfg->sensor_count; i++) {
    pstSensorCfg = RKADK_PARAM_GetSensorCfg(i);
    if (!pstSensorCfg) {
      RKADK_LOGE("RKADK_PARAM_GetSensorCfg[%d] failed", i);
      continue;
    }

    if (u32AencChnId == pstSensorCfg->aenc_chn) {
      RKADK_LOGE("rtsp aenc chn[%d] conflict with sensor[%d] aenc chn[%d]", u32AencChnId, i, pstSensorCfg->aenc_chn);
      return -1;
    }
  }
  ...
  pstAencChn->s32ChnId = u32AencChnId;
  return 0;
}
...
static void RKADK_RTSP_VencOutCb(RKADK_MEDIA_VENC_DATA_S mb, RKADK_VOID *handle) {
  ...
  if (!pHandle->bWaitIDR) {
    ...
    if (pHandle->u64FirstVideoPts == 0) {
      RKADK_KLOG("Rtsp first key frame pts: %lld", stData.stFrame.pstPack->u64PTS);
      pHandle->u64FirstVideoPts = stData.stFrame.pstPack->u64PTS;
    }
  }
...
static void RKADK_RTSP_AencOutCb(AUDIO_STREAM_S stFrame,
  ...
  if (pstHandle->u64FirstVideoPts == 0) {
    return;
  }

  if (pstHandle->s64FirstDiffPts == 0)
    pstHandle->s64FirstDiffPts = pstHandle->u64FirstVideoPts - stFrame.u64TimeStamp;
  ...
  if (pstHandle->stRtspHandle && pstHandle->stRtspSession) {
    ...
    rtsp_tx_audio(pstHandle->stRtspSession, (uint8_t *)data,
                  stFrame.u32Len, stFrame.u64TimeStamp + pstHandle->s64FirstDiffPts);
...
RKADK_S32 RKADK_RTSP_Init(RKADK_U32 u32CamId, RKADK_U32 port, const char *path,
  ...
  RKADK_LOGI("Rtsp[%d, %d, %s] Init Start...", u32CamId, port, path);
  ...
  if (RKADK_RTSP_AudioSetChn(&stAiChn, &stAencChn)) {
    RKADK_LOGE("RKADK_RTSP_AudioSetChn failed");
    free(pHandle);
    return -1;
  }
  ...
  RKADK_PARAM_SetVAdvancedParam(u32CamId, RKADK_STREAM_TYPE_LIVE, pstLiveCfg->attribute);

  //if use isp, set mirror/flip using aiq
  if (!pstSensorCfg->used_isp) {
    if (pstSensorCfg->mirror)
      RKADK_MEDIA_ToggleVencMirror(u32CamId, RKADK_STREAM_TYPE_LIVE, pstSensorCfg->mirror);
    if (pstSensorCfg->flip)
      RKADK_MEDIA_ToggleVencFlip(u32CamId, RKADK_STREAM_TYPE_LIVE, pstSensorCfg->flip);
  }
...
RKADK_S32 RKADK_RTSP_DeInit(RKADK_MW_PTR pHandle) {
  ...
  RKADK_LOGI("Rtsp[%d] DeInit Start...", pstHandle->u32CamId);
