...
int sample_common_isp_init(int cam_id, rk_aiq_working_mode_t WDRMode, bool MultiCam,
	...
	snprintf(entry, 127, "isp.%d.adjustment:force_iq_name", cam_id);
	const char *force_iq_name = rk_param_get_string(entry, NULL);
	if (force_iq_name) {
		memset(entry, 0, sizeof(entry));
		snprintf(entry, 127, "isp.%d.adjustment:force_hdr_mode", cam_id);
		rk_aiq_working_mode_t force_hdr_mode =
		    (rk_aiq_working_mode_t)rk_param_get_int(entry, RK_AIQ_WORKING_MODE_NORMAL);
		rk_aiq_uapi2_sysctl_preInit(aiq_static_info.sensor_info.sensor_name, force_hdr_mode,
		                            force_iq_name);
		LOG_INFO("preinit hdr mode id %d, iq name is %s\n", force_hdr_mode, force_iq_name);
	}
	int ret;
...
// LOG_ERROR("tmp force use m00_b_imx464 3-001a\n");
	...
	btnr_attr.stAuto.sta.swMemc.sw_btnr_swMemc_en = rk_param_get_int("video.source:enable_memc", 0);
	btnr_attr.stMan.sta.swMemc.sw_btnr_swMemc_en = rk_param_get_int("video.source:enable_memc", 0);
...
int rk_isp_set_hdr(int cam_id, const char *value) {
	...
	int enable_ivs = rk_param_get_int("video.source:enable_ivs", 1);
	int g_enable_vo = rk_param_get_int("video.source:enable_vo", 1);
	...
	if (strcmp(value, old_value)) {
		if (rkipc_aiq_use_group) {
			...
			sensor_num = rk_param_get_int("avs:sensor_num", 2);
			for (int i = 0; i < sensor_num; i++) {
				ret |= RK_MPI_VI_PauseChn(i, 3);
				if (g_enable_vo)
					ret |= RK_MPI_VI_PauseChn(i, 4);
			}
			...
			for (int i = 0; i < sensor_num; i++) {
				ret |= RK_MPI_VI_ResumeChn(i, 3);
				if (g_enable_vo)
					ret |= RK_MPI_VI_ResumeChn(i, 4);
			}
		} else {
			...
			if (!strcmp(distortion_correction, "FEC")) {
				...
				if (enable_venc_0)
					RK_MPI_VI_PauseChn(pipe_id, 3);
				if (enable_venc_1)
					RK_MPI_VI_PauseChn(pipe_id, 2);
				if (enable_npu || enable_ivs)
					RK_MPI_VI_PauseChn(pipe_id, 4);
				if (g_enable_vo)
					RK_MPI_VI_PauseChn(pipe_id, 5);
			}
			...
			if (!strcmp(distortion_correction, "FEC")) {
				...
				if (enable_venc_0)
					RK_MPI_VI_ResumeChn(pipe_id, 3);
				if (enable_venc_1)
					RK_MPI_VI_ResumeChn(pipe_id, 2);
				if (enable_npu || enable_ivs)
					RK_MPI_VI_ResumeChn(pipe_id, 4);
				if (g_enable_vo)
					RK_MPI_VI_ResumeChn(pipe_id, 5);
			}
		}
	}
...
int rk_isp_set_distortion_correction(int cam_id, const char *value) {
	...
	char entry[128] = {'\0'};
	snprintf(entry, 127, "isp.%d.enhancement:distortion_correction", cam_id);
	LOG_INFO("old_value is %s, new value is %s\n", old_value, value);
...
int rk_isp_set_fec_level(int cam_id, int value) {
	int ret;
	GDC_UPDATE_ATTR_S attr;
	const char *correction_mode;
	RK_ISP_CHECK_CAMERA_ID(cam_id);

	if (rk_param_get_int("avs:enable_avs", 0)) {
		LOG_INFO("avs not set fec level\n");
		return 0;
	}

	rk_isp_get_distortion_correction(cam_id, &correction_mode);
	if (strcmp(correction_mode, "FEC")) {
		LOG_INFO("correction_mode is %s, can't set fec level\n", correction_mode);
		return 0;
	}

	RK_MPI_GDC_GetUpdateAttr(0, &attr);
	attr.stFecAttr.stOnlineCfg.s32CorrectLevel = (int)(value * 2.55); // [0-100] -> [0->255]
	ret = RK_MPI_GDC_Update(0, &attr);
	if (ret)
		LOG_ERROR("RK_MPI_GDC_Update fail, ret = %x\n", ret);
	char entry[128] = {'\0'};
	snprintf(entry, 127, "isp.%d.enhancement:fec_level", cam_id);
	rk_param_set_int(entry, value);
...
int rk_isp_set_ldch_level(int cam_id, int value) {
	...
	if (rk_param_get_int("avs:enable_avs", 0)) {
		LOG_INFO("avs not set ldch level\n");
		return 0;
	}
...
void *rk_isp_get_ctx(int cam_id) { return g_aiq_ctx[cam_id]; }
