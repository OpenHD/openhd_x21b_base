...
#ifdef RV1126B
    pstVpssGrpAttr->enVProcDev = (VIDEO_PROC_DEV_TYPE_E)pstCommCfg->vpss_devcie;
#endif
...
RKADK_S32 RKADK_MPI_SYS_Init() {
  ...
  RKADK_LOGI("System init success");
  return 0;
}
...
RKADK_S32 RKADK_MPI_SYS_Exit() {
  ...
  RKADK_LOGI("System deinit success");
  return 0;
}
...
static RKADK_S32 RKADK_MPI_VI_DevInit(RKADK_U32 u32CamId, RKADK_U32 u32DevId) {
  int ret = -1;
  ...
  if (u32DevId >= VI_MAX_PHY_PIPE_NUM) {
    RKADK_LOGD("u32CamId[%d] u32DevId[%d] don't need enable device, VI_MAX_PHY_PIPE_NUM[%d]", u32CamId, u32DevId, VI_MAX_PHY_PIPE_NUM);
    return 0;
  }
  ...
  if (ret == RK_ERR_VI_NOT_CONFIG) {
    ...
    if (ret) {
      RKADK_LOGE("CamId[%d] RK_MPI_VI_SetDevAttr[%d] failed[%x]", u32CamId, u32DevId, ret);
      return ret;
    }
  } else {
    RKADK_LOGI("CamId[%d] RK_MPI_VI_SetDevAttr[%d] already", u32CamId, u32DevId);
  }
  ...
  if (ret) {
    ...
    if (ret) {
      RKADK_LOGE("CamId[%d] RK_MPI_VI_EnableDev[%d] failed[%x]", u32CamId, u32DevId, ret);
      return ret;
    }
    ...
    if (ret) {
      RKADK_LOGE("CamId[%d] RK_MPI_VI_SetDevBindPipe[%d] failed[%x]", u32CamId, u32DevId, ret);
    ...
    if (ret)
      RKADK_LOGE("CamId[%d] RK_MPI_VI_SetModParam[%d] failed[%x]", u32CamId, u32DevId, ret);

    memset(&stModParam, 0, sizeof(stModParam));
    stModParam.enViModType = VI_EXT_CHN_MODE;
    ret = RK_MPI_VI_GetModParam(&stModParam);
    if (ret)
      RKADK_LOGE("CamId[%d] RK_MPI_VI_GetModParam[%d] failed[%x]", u32CamId, u32DevId, ret);

    RKADK_LOGP("CamId[%d] vi[%d] mod:%d mirror:%d ext_chn_mode:%d ext_chn1_mode:%d, "
            "ext_chn2_mode:%d, ext_chn3_mode:%d, ext_chn4_mode:%d, ext_chn5_mode:%d",
            u32CamId, u32DevId, stModParam.enViModType, stModParam.stExtChnParam.mirrorCmsc,
    ...
    RKADK_LOGI("CamId[%d] RK_MPI_VI_EnableDev[%d] already", u32CamId, u32DevId);
  }

  return 0;
}

static RKADK_S32 RKADK_MPI_VI_DevDeInit(RKADK_U32 u32CamId, RKADK_U32 u32DevId) {
  if (u32DevId >= VI_MAX_PHY_PIPE_NUM) {
    RKADK_LOGD("u32CamId[%d] u32DevId[%d] don't need disable device, VI_MAX_PHY_PIPE_NUM[%d]", u32CamId, u32DevId, VI_MAX_PHY_PIPE_NUM);
    return 0;
  }
...
RKADK_S32 RKADK_MPI_VI_Init(RKADK_U32 u32CamId, RKADK_S32 s32ViChnId,
  ...
  if (0 == g_stMediaCtx.stViInfo[i].s32InitCnt) {
    ret = RKADK_MPI_VI_DevInit(u32CamId, u32DevId);
    if (ret) {
      RKADK_LOGE("CamId[%d] VI Device[%d] Init failed[%x]", u32CamId, u32DevId, ret);
      goto exit;
    }

    ret = RK_MPI_VI_SetChnAttr(u32DevId, s32ViChnId, pstViChnAttr);
    if (ret) {
      RKADK_LOGE("CamId[%d] Set VI[%d, %d] attribute failed[%x]", u32CamId, u32DevId, s32ViChnId, ret);
      RKADK_MPI_VI_DevDeInit(u32CamId, u32DevId);
      goto exit;
    }
    ...
    if (s32ViChnId == 0 && pstSensorCfg->enable_wrap) {
      ...
      RKADK_LOGD("CamId[%d] vi[%d, %d] set channel wrap line[%d] wrapBuffSize[%d]",
                u32CamId, u32DevId, s32ViChnId, stViWrap.u32BufLine, stViWrap.u32WrapBufferSize);
      RK_MPI_VI_SetChnWrapBufAttr(u32DevId, s32ViChnId, &stViWrap);
    }

    ret = RK_MPI_VI_EnableChn(u32DevId, s32ViChnId);
    if (ret) {
      RKADK_LOGE("CamId[%d] Create VI[%d, %d] failed[%x]", u32CamId, u32DevId, s32ViChnId, ret);
      RKADK_MPI_VI_DevDeInit(u32CamId, u32DevId);
      goto exit;
    }
    ...
    g_stMediaCtx.stViInfo[i].s32DevId = u32DevId;
  }

  g_stMediaCtx.stViInfo[i].s32InitCnt++;
  RKADK_LOGD("CamId[%d] viChnId[%d, %d], InitCnt[%d]", u32CamId, u32DevId, s32ViChnId,
             g_stMediaCtx.stViInfo[i].s32InitCnt);
  ret = 0;
...
RKADK_S32 RKADK_MPI_VI_DeInit(RKADK_U32 u32CamId, RKADK_S32 s32ViChnId) {
  ...
  u32DevId = RKADK_PARAM_GetViDevId(u32CamId);
  if (u32DevId == -1) {
    RKADK_LOGE("RKADK_PARAM_GetViDevId failed");
    return -1;
  }
  ...
  if (0 == s32InitCnt) {
    RKADK_LOGD("CamId[%d] vi[%d, %d] has already deinit", u32CamId, u32DevId, s32ViChnId);
    RKADK_MUTEX_UNLOCK(g_stMediaCtx.viMutex);
    return 0;
  } else if (1 == s32InitCnt) {
    ret = RK_MPI_VI_DisableChn(u32DevId, s32ViChnId);
    if (ret) {
      RKADK_LOGE("CamId[%d] Destory VI[%d, %d] failed[%x]", u32CamId, u32DevId, s32ViChnId, ret);
      goto exit;
    }
    ...
    if (j == RKADK_MEDIA_VI_MAX_CNT) {
      ret = RKADK_MPI_VI_DevDeInit(u32CamId, u32DevId);
      if (ret) {
        RKADK_LOGE("CamId[%d] RK_MPI_VI_DisableDev[%d] failed[%x]", u32CamId, u32DevId, ret);
        goto exit;
      }
      RKADK_LOGD("CamId[%d] RK_MPI_VI_DisableDev[%d] done", u32CamId, u32DevId);
    }
...
RKADK_S32 RKADK_MPI_VI_QueryCameraStatus(RKADK_U32 u32CamId) {
  ...
  ret = RK_MPI_VI_QueryDevStatus(u32DevId, &stDevStatus);
  if (ret == RK_SUCCESS) {
    if (!stDevStatus.bProbeOk) {
      RKADK_LOGE("Camera[%d] u32DevId[%d] sensor probe failed[%x]", u32CamId, u32DevId, ret);
      return -1;
    }
  }
...
RKADK_VOID RKADK_MPI_VO_CheckDefaultID(RKADK_S32 *VoDev, RKADK_S32 *VoLayer) {
  if (VoDev && *VoDev == -1) {
#if defined (RK3576)
    *VoDev = 1;
#else
...
RKADK_S32 RKADK_MPI_SYS_Bind(MPP_CHN_S *pstSrcChn,
  ...
  u32DevId = pstSrcChn->s32DevId;
  if (pstSrcChn->enModId == RK_ID_VI) {
    pstSensorCfg = RKADK_PARAM_GetSensorCfg(u32DevId);
    if (!pstSensorCfg) {
      RKADK_LOGE("RKADK_PARAM_GetSensorCfg[%d] failed", u32DevId);
      goto exit;
    }
    pstSrcChn->s32DevId = pstSensorCfg->dev_id;
    RKADK_LOGD("VI module CamId[%d] -> sensor DevId[%d]", u32DevId, pstSensorCfg->dev_id);
  }
  ...
  if (0 == pstInfo[i].s32BindCnt) {
    ...
    if (ret) {
      RKADK_LOGE("Bind src[%d(%d), %d, %d] and dest[%d, %d, %d] failed[%x]",
                  pstSrcChn->s32DevId, u32DevId, pstSrcChn->enModId, pstSrcChn->s32ChnId,
  ...
  RKADK_LOGD("src[%d(%d), %d, %d], dest[%d, %d, %d], BindCnt[%d]",
              pstSrcChn->s32DevId, u32DevId, pstSrcChn->enModId, pstSrcChn->s32ChnId,
...
exit:
  pstSrcChn->s32DevId = u32DevId;
  RKADK_MUTEX_UNLOCK(g_stMediaCtx.bindMutex);
  return ret;
}

RKADK_S32 RKADK_MPI_SYS_UnBind(MPP_CHN_S *pstSrcChn,
                               MPP_CHN_S *pstDestChn) {
  int ret = -1, count;
  RKADK_S32 i;
  RKADK_U32 u32DevId = 0;
  ...
  if (0 == pstInfo[i].s32BindCnt) {
    ...
    if (ret) {
      RKADK_LOGE("UnBind src[%d(%d), %d, %d] and dest[%d, %d, %d] failed[%x]",
                  pstSrcChn->s32DevId, u32DevId, pstSrcChn->enModId, pstSrcChn->s32ChnId,
                  pstDestChn->s32DevId, pstDestChn->enModId,
                  pstDestChn->s32ChnId, ret);
      goto exit;
...
int RKADK_MEDIA_GetPixelFormat(PIXEL_FORMAT_E enPixelFormat,
  ...
  case RK_FMT_YUV422_YUYV:
    memcpy(cPixFmt, "YUYV", strlen("YUYV"));
    break;
  case RK_FMT_YUV422_YVYU:
    memcpy(cPixFmt, "YVYU", strlen("YVYU"));
    break;
  default:
    memcpy(cPixFmt, "NV12", strlen("NV12"));
    RKADK_LOGE("default pixel format: %s", cPixFmt);
    ret = -1;
    break;
  }
...
#if !defined(RV1106_1103) && !defined(RV1103B)
    RKADK_LOGD("only rv1106/rv1103b support dynamic setting code type, Old type [%d], new type [%d]",
...
VIDEO_MODE_E RKADK_MEDIA_GetRkDecodeMode(RKADK_VDEC_DECODE_MODE_E enMode) {
