...
int RKADK_AOV_Init(RKADK_AOV_ARG_S *pstAovAttr) {
  ...
  if (ret) {
    RKADK_LOGE("aov mutex init failed[%d]", ret);
    return -1;
  }
...
int RKADK_AOV_ReadReg(int addr, unsigned char *buf, int len) {
  ...
  if (len > page_size) {
    RKADK_LOGE("Dump length is too long!");
    return -1;
  }
...
int RKADK_AOV_WriteReg(int addr, int value) {
