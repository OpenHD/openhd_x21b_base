...
| V1.6.9      | Fenrir Lin | 2024-08-29 | Modify the RV1106 Dual-IPC block diagram.                    |
| V1.7.0      | Fenrir Lin | 2025-01-06 | Added RK3576 DV block diagram.                               |
| V1.7.1      | Yu Zheng   | 2025-02-12 | Modified RK3576DV flow block diagram and part of the parameter description <br/>Added playback and display module API introduction |
| V1.7.2      | Fenrir Lin | 2025-03-25 | Add RV1126B IPC block diagram.                               |
| V1.7.3      | Fenrir Lin | 2025-04-03 | Add RV1126B Dual-IPC block diagram.                          |
| V1.7.4      | Yu Zheng   | 2025-04-16 | Add RV1126B DV block diagram.                                |
| V1.7.5      | Fenrir Lin | 2025-04-24 | Add UVC module related instructions.                         |
...
| rk3576_dv                 | rockit, rkaiq         | For sports DV products on the RK3576 platform, it supports local photography/recording, decoding playback, and screen display. |
| rv1126b_ipc               | rockit, rkaiq         | Single camera IPC product for the RV1126B platform, supports web and rtsp/rtmp preview, dynamic parameter modification. |
| rv1126b_dual_ipc          | rockit, rkaiq         | Multi cameras IPC product for RV1126B platform, supports web and rtsp/rtmp preview, dynamic parameter modification. |
| rv1126b_dv                | rockit, rkaiq         | For sports DV products on the RV1126B platform, it supports local photography/recording, decoding playback, and screen display. |
...
graph LR
	AI-->AENC-->RTSP_RTMP_0
	AENC-->MUXER
	VI_0-->GDC_0-->VENC_0_normal-->MUXER-->MP4
	GDC_0-.->VENC_1_slowmotion-.->MUXER
	GDC_0-.->VENC_2_timelapse-.->MUXER
	VENC_0_normal-->RTSP_RTMP_0
	GDC_0-.->VENC_3-.->JPEG
	VI_1-->GDC_1-->VO
```
...
**It is "FEC close" by default, when isp.0.enhancement: distortion_correction=FEC, it will follow a different data flow.**

```mermaid
graph LR
AI-->AENC-->RTSP_RTMP_0

subgraph "FEC close, VPSS online"
    VI_PIPE-->VI_ext_3
    VI_PIPE-->VI_ext_4
    VI_PIPE-->VI_ext_2
    VI_PIPE-->VI_ext_5
end
VI_ext_3--RFBC_or_WRAP-->VENC_0_draw_nn_result-->MUXER-->MP4
VENC_0_draw_nn_result-->UVC
VENC_0_draw_nn_result--combo-->JPEG
VI_ext_4-->NPU
VI_ext_4-->IVS_for_OD_MD
VENC_0_draw_nn_result-->RTSP_RTMP_0
VI_ext_2-->VENC_1-->RTSP_RTMP_1
VI_ext_5-->VO

subgraph "FEC open, VPSS offline"
	VI_0-.TILE.->GDC-.RFBC.->VPSS_GRP
	VPSS_GRP-.->VPSS_CHN_0
	VPSS_GRP-.->VPSS_CHN_1
	VPSS_GRP-.->VPSS_CHN_2
	VPSS_GRP-.->VPSS_CHN_3
end
VPSS_CHN_0-.RFBC.->VENC_0_draw_nn_result
VPSS_CHN_2-.->NPU
VPSS_CHN_2-.->IVS_for_OD_MD
VPSS_CHN_1-.->VENC_1
VPSS_CHN_3-.->VO

AIISP-.->VI_PIPE
AIISP-.->VI_0
cover/mosaic-->VI_PIPE
cover/mosaic-.->VPSS_GRP
```
...
```mermaid
graph LR
       VI_0_0-->VENC_0-->RTSP_0
       VI_0_4-->VO
       VI_1_0-->VENC_1-->RTSP_1
       VI_1_4-->VO
```
...
graph LR
	AI-->AENC-->RTSP_RTMP_0
	AENC-->MUXER
	VI_0--TILE-->GDC_0--FBC-->VENC_0_normal-->MUXER-->MP4
	GDC_0-.->VENC_3-.->JPEG
	VENC_0_normal-->RTSP_RTMP_0
	VI_1-->GDC_1-->VO
```
...
│   │   └── SourceHanSansEN.ttf # Font library
...
### Playback and display module

| Function Name                | Functionality                                    |
| ---------------------------- | ------------------------------------------------ |
| rk_player_create             | Create playback module                           |
| rk_player_destroy            | Destroy playback module                          |
| rk_player_play               | Start playing                                    |
| rk_player_pause              | Pause playing                                    |
| rk_player_stop               | Stop playing                                     |
| rk_player_set_file           | Set the file to play                             |
| rk_player_get_photo          | Parse a JPEG image                               |
| rk_player_release_photo      | Frees the resources used to parse JPEG           |
| rk_player_video_seek         | Set the time from which the video starts playing |
| rk_player_get_video_duration | Get the total duration of the video              |
| rk_player_get_video_position | Get the current position of the video            |
| rk_player_audio_seek         | Set the time from which the audio starts playing |
| rk_player_get_audio_duration | Get the total duration of the audio              |
| rk_player_get_audio_position | Get the current position of the audio            |

### UVC Module

The important script is located at: common\uvc\rkipc_usb_config.sh, By default, only one UVC is run, and only one resolution and format are supported. If multiple channels and formats are required. Please refer to the following judgment in the script and add them yourself.

```shell
#uvc config init
uvc_device_config uvc.gs1 "UVC RGB" $FORMAT $WIDTH $HEIGHT
if [ $UVC_MULTI = ONE ];then
   uvc_device_config uvc.gs2 "RK UVC" "H.264" "1920" "1080"
elif [ $UVC_MULTI = TWO ];then
   uvc_device_config uvc.gs2 "RK UVC 1" "H.264" "1920" "1080"
   uvc_device_config uvc.gs3 "RK UVC 2" "H.264" "1920" "1080"
fi
```
...
// init
system("/etc/init.d/S50usbdevice stop"); // stop old usb device
sleep(2); // wait old usb change
system("rkipc_usb_config.sh -f H.264 -w 1920 -h 1080");
sleep(1);
...
while (g_video_run_) {
	...
	if (ret == RK_SUCCESS) {
		...
		RK_MPI_VENC_ReleaseStream(VIDEO_PIPE_0, &stFrame);
	}
}
uvc_read_camera_buffer(data, -1, stFrame.pstPack->u32Len, NULL, 0);
...
width = 2560
height = 1440
rc_mode = VBR ; Rate control mode
rc_quality = highest ; Rate control quality
...
min_rate = 200 ; Min bitrate, please see rockit documentation for details
output_data_type = H.265 ; Video encoding type
smart = open ; Enable smart encoding, Please note that it is not smartp
...
; The following are advanced encoding parameters. Please add them as needed. For detailed instructions, please refer to the rockit and mpp documents
...
font_path = /oem/usr/share/SourceHanSansEN.ttf ; Font library path
...
```ini
[display]
dev_id = 1 ; Display device id
layer_id = 5 ; Display layer id
width = 240 ; Display width
height = 320 ; Display height
fps = 30 ; Display framerate
intf_type = LCD ; Display device type
play_chn_id = 1 ; Display channel id
play_chn_priority = 2 ; The priority of the playback channel. The channel with higher priority overwrites the channel with lower priority
```
