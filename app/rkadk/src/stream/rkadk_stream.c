...
RKADK_S32 RKADK_STREAM_VideoInit(RKADK_STREAM_VIDEO_ATTR_S *pstVideoAttr,
  ...
  RKADK_PARAM_SetVAdvancedParam(pVideoHandle->u32CamId, RKADK_STREAM_TYPE_PREVIEW, pstStreamCfg->attribute);

  //if use isp, set mirror/flip using aiq
  if (!pstSensorCfg->used_isp) {
    if (pstSensorCfg->mirror)
      RKADK_MEDIA_ToggleVencMirror(pstVideoAttr->u32CamId, RKADK_STREAM_TYPE_PREVIEW, pstSensorCfg->mirror);
    if (pstSensorCfg->flip)
      RKADK_MEDIA_ToggleVencFlip(pstVideoAttr->u32CamId, RKADK_STREAM_TYPE_PREVIEW, pstSensorCfg->flip);
  }
...
static RKADK_S32 RKADK_STREAM_SetAencConfig(RKADK_U32 u32CamId,
  ...
  pstHandle->stAencChn.s32ChnId = pstSensorCfg->aenc_chn;
  return 0;
}
