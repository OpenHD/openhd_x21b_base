...
#define FONT_LIBRARY_PATH "/usr/share/SourceHanSansEN.ttf"

#define MAX_OSD_NUM 8

static int g_osd_server_run_ = 1;
static int g_osd_time_server_run_ = 1;
...
int generate_date_time(const char *fmt, wchar_t *result) {
	...
	if (strstr(fmt, OSD_FMT_WEEK0)) {
		...
		ret = snprintf(week_string, sizeof(week) - 1, " %s", week);
		if (ret >= sizeof(week) - 1)
			LOG_ERROR("%s: truncated buffer!", __func__);
	}
...
static void *osd_time_server(void *arg) {
	...
	if(rk_osd_bmp_create_(osd_time_id, &osd_data)) {
		LOG_ERROR("rk_osd_bmp_create_ fail\n");
		g_osd_time_server_run_ = 0;
		free(osd_data.buffer);
		return NULL;
	}
	g_osd_time_server_run_ = 1;
	...
	for (int i = 0; i < MAX_OSD_NUM; i++) {
		...
		if (!strcmp(osd_type, "image")) {
			...
			if (!strcmp(osd_type, "channelName") || !strcmp(osd_type, "character")) {
				...
				// judge image depend on ini or json
				if (display_text && !strcmp(display_text, "isp config:")) {
					strncpy(temp_text, display_text, sizeof(temp_text) - 1);
					int init_from_ini = rk_param_get_int("isp:init_from_ini", 1);
					strcat(temp_text, init_from_ini ? "ini" : "json");
					display_text = temp_text;
					LOG_INFO("display_text is %s\n", display_text);
				}
