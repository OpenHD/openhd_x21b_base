...
static RKADK_CHAR optstr[] = "i:x:y:W:H:r:a:s:P:I:t:F:T:l:c:d:O:S:V:R:w:C:A:M:N:B:mfvbDhE";
...
static void print_usage(const RKADK_CHAR *name) {
  ...
  printf("\t-I: Type of a VO interface, option: 0(DEFAILT), 1(MIPI), 2(LCD); Default: RV1106/RK3506: 0, other chip: 1\n");
  ...
  printf("\t-C: Ao sound card name, Default: RV1106/RV1103B/RK3506/RV1126B = hw:0,0, other chip = default\n");
  ...
  printf("\t-B: fade in config frames, Default: 0\n");
  printf("\t-E: enable smooth loop play, need config loop play count when enable, Default: disable\n");
  printf("\t-h: help\n");
}
...
int main(int argc, char *argv[]) {
  ...
  RKADK_U32 fadeInFrames = 0;
  RKADK_U32 smooth_loop = 0;

  memset(&stPlayCfg, 0, sizeof(RKADK_PLAYER_CFG_S));
  param_init(&stPlayCfg.stFrmInfo);

#if defined(RK3506) || defined(RV1106_1103) || defined(RV1103B) || defined(RV1126B)
  ...
  while ((c = getopt(argc, argv, optstr)) != -1) {
    ...
    case 'N':
      ...
      break;
    case 'B':
      stPlayCfg.stFadeInCfg.u32FadeInFrames = atoi(optarg);
      stPlayCfg.stFadeInCfg.u32VoChn = 2;
      break;
    case 'E':
      smooth_loop = 1;
      break;
  ...
  if (smooth_loop)
    RKADK_PLAYER_SmoothPlayEnable(pPlayer);
    ...
    if (loop_count >= 0 && !stPlayCfg.bEnableThirdDemuxer) {
      if (smooth_loop) {
        if (loop_count == 0) {
          ...
          RKADK_PLAYER_SmoothPlayDisable(pPlayer);
          RKADK_LOGP("smooth loop play end!");
          is_quit = true;
          goto __EXIT;
        }

        RKADK_PLAYER_SmoothPlay(pPlayer, file);
        RKADK_LOGP("smooth loop play test, loop_count: %d", loop_count);
        loop_count--;
        continue;
      }
      ...
      } else if (strstr(cmd, "fade")) {
        fgets(cmd, sizeof(cmd), stdin);
        fadeInFrames = atoi(cmd);
        RKADK_PLAYER_SetFadeInFrames(pPlayer, fadeInFrames);
        RKADK_PLAYER_Stop(pPlayer);
        ret = RKADK_PLAYER_SetDataSource(pPlayer, file);
        if (ret) {
          RKADK_LOGE("SetDataSource failed, ret = %d", ret);
...
static void playerMain(void *arg) {
  ...
  tmp_loop_count = loop_count;
  while (loop_count) {
    if (smooth_loop && loop_count < tmp_loop_count) {
      if (loop_count == 0) {
        RKADK_PLAYER_GetPlayStatus(pPlayer, &enState);
        if (loop_duration > 0) {
          sleep(loop_duration);
    ...
    if (!smooth_loop) {
      ...
      if (loop_count == tmp_loop_count)
        rkos_msleep(loop_duration);
    }
  }
