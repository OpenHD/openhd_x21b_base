...
RKADK_S32 RKADK_UI_Create(RKADK_UI_ATTR_S *pstUiAttr, RKADK_MW_PTR *ppUi) {
  ...
  stChnAttr.u32Priority = pstUiAttr->u32VoChn;
  ...
  RKADK_MPI_VO_CheckDefaultID((RKADK_S32 *)&pstUiAttr->u32VoDev, (RKADK_S32 *)&pstUiAttr->u32VoLay);
...
RKADK_S32 RKADK_UI_Update(RKADK_MW_PTR pUi, RKADK_UI_FRAME_INFO *pstUiFrameInfo) {
  ...
  if (pstUiFrameInfo->s32MilliSec != 0)
    s32MilliSec = pstUiFrameInfo->s32MilliSec;
  ...
  ret = RK_MPI_VO_SendFrame(pstHandle->u32VoLay, pstHandle->u32VoChn, &stVoVFrame, s32MilliSec);
  if(ret) {
