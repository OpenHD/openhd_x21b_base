...
post_chk()
{
	...
	if [ ! -f "/usr/share/rkipc.ini" ]; then
		...
		if [ "$resolution" = "1920x1080" ] ;then
			ln -s -f /usr/share/rkipc-1920x1080.ini $default_rkipc_ini
		fi
		if [ "$resolution" = "2688x1520" ] ;then
			ln -s -f /usr/share/rkipc-2688x1520.ini $default_rkipc_ini
		fi
		if [ "$resolution" = "3200x1800" ] ;then
			ln -s -f /usr/share/rkipc-3200x1800.ini $default_rkipc_ini
		fi
		if [ "$resolution" = "3840x2160" ] ;then
			ln -s -f /usr/share/rkipc-3840x2160.ini $default_rkipc_ini
		fi
	fi
