...
static void RKADK_RTMP_AudioSetChn(RKADK_U32 u32CamId, MPP_CHN_S *pstAiChn, MPP_CHN_S *pstAencChn) {
...
RKADK_S32 RKADK_RTMP_Init(RKADK_U32 u32CamId, const char *path,
  ...
  if (bEnableAudio) {
    RKADK_RTMP_AudioSetChn(u32CamId, &stAiChn, &stAencChn);
...
RKADK_S32 RKADK_RTMP_DeInit(RKADK_MW_PTR pHandle) {
  ...
  bDisableAudio = pstAudioParam->codec_type == RKADK_CODEC_TYPE_ACC ? true : false;
  ...
  RKADK_MEDIA_StopGetVencBuffer(pstHandle->u32CamId, &stVencChn, false, RKADK_RTMP_VencOutCb, pstHandle);
   ...
   // Stop get aenc data
  if (bDisableAudio) {
    RKADK_RTMP_AudioSetChn(pstHandle->u32CamId, &stAiChn, &stAencChn);
